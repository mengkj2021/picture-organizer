...
> 「图片整理」App 技术架构说明。版本矩阵见 [项目式样说明书.md](../式样/项目式样说明书.md) §3.1；包结构约定见 [shared/rules/dev-convention.md](../../shared/rules/dev-convention.md)。完整 route 见 [路由设计.md](路由设计.md)。
...
| 数据访问 | Repository 模式 |
| 持久化 | Room 2.7.2（v3：`images` + `tags` + `tag_templates` + `rename_templates`） |
| 偏好 | DataStore Preferences（教程完成、默认标签、导入压缩开关） |
| 文件 / 图片 | `AppFileManager` / `ImageManager` / `ImageTagMetadata` / `ZipExporter` / `RenamePatternApplier` |
...
│   └── mock/
├── util/
│   ├── file/                   # AppFileManager、ZipExporter、RenamePatternApplier
│   └── image/                  # ImageManager、ImageTagMetadata
└── ui/
    ├── splash/ / tutorial/
    ├── main/ / filter/
...
相册 / 文件多选 → 读压缩开关与默认标签 → `ImageManager.prepareForImport(uri, compressEnabled)`（关压缩则原样复制）→ 落盘 `pending/` → `ImageRepository.insert`（Pending，合并默认标签进 `tagsJson`）→ 返回 main（Flow 自动刷新）
...
| `UserPreferencesRepository` | DataStore：`tutorial_completed`、`default_tag_names`、`import_compress_enabled` |

## 5. 工具类与系统组件

| 类 / 组件 | 职责 |
|---|---|
| `AppFileManager` | `images/{pending,confirmed,no_modify}/`、URI 复制、跨状态搬移、同目录重命名、删除；`exportsDir()` |
...
6. 工程质量与体验增强：见 [改动清单](../工程/改动清单.md)（余 **B1 / B2** 与 **P** 项）
