...
相册 / 文件多选 → 读压缩开关与默认标签 → **F7** 压缩前从源 Uri 读 Exif `UserComment` 用户标签 → `ImageManager.prepareForImport(uri, compressEnabled)`（关压缩则原样复制）→ 落盘 `pending/` → `ImageRepository.insert`（Pending，`tagsJson` = 默认标签 ∪ Exif 回读去重）→ 回读/默认标签进词表（对齐 S2）→ 返回 main（Flow 自动刷新）
