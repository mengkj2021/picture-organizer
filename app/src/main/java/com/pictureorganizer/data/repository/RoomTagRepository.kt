...
class RoomTagRepository(
    ...
    override fun observeTags(): Flow<List<Tag>> {
        return tagDao.observeAll().map { list -> list.map { it.toModel() } }
    }

    override fun observeTemplates(): Flow<List<TagTemplate>> {
        return tagTemplateDao.observeAll().map { list -> list.map { it.toModel() } }
    }

    override suspend fun getTags(): List<Tag> = tagDao.getAll().map { it.toModel() }

    override suspend fun getTemplates(): List<TagTemplate> =
        tagTemplateDao.getAll().map { it.toModel() }

    override suspend fun getTag(id: String): Tag? = tagDao.findById(id)?.toModel()

    override suspend fun getTemplate(id: String): TagTemplate? =
        tagTemplateDao.findById(id)?.toModel()

    override suspend fun insertTag(tag: Tag) = withContext(Dispatchers.IO) {
    ...
    override suspend fun insertTemplate(template: TagTemplate) = withContext(Dispatchers.IO) {
    ...
    override suspend fun updateTemplate(template: TagTemplate) = withContext(Dispatchers.IO) {
        val name = template.name.trim()
        require(name.isNotEmpty()) { "模板名不能为空" }
    ...
    override suspend fun deleteTemplate(id: String) = withContext(Dispatchers.IO) {
        tagTemplateDao.deleteById(id)
    }

    override suspend fun setDefaultTemplate(id: String) = withContext(Dispatchers.IO) {
