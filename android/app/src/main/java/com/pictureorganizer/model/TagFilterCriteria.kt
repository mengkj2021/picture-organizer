...
enum class ImageListSort {
    ...
    /** 文件名 A→Z（按 description / 路径末段，忽略大小写） */
    NameAsc,
    ;

    companion object {
        fun fromParam(raw: String?): ImageListSort =
            entries.firstOrNull { it.name.equals(raw, ignoreCase = true) } ?: ImportedAtDesc
    }
}

/**
 * 标签 / 文件名筛选条件（主画面一览 / 筛选画面 / 打包共用）。
 * 标签多选为 OR；[includeUntagged] 表示无用户标签的项。
 * F6：[nameContains] 匹配文件名或 description（忽略大小写）；[sort] 控制排序。
 */
