...
object MockImageRepository : ImageRepository {
    ...
    override fun observeItems(status: ImageStatus): Flow<List<ImageListItem>> = items.map { list -> list.filter { it.status == status } }

    override fun observeItem(id: String): Flow<ImageListItem?> = items.map { list -> list.find { it.id == id } }

    override suspend fun getItems(status: ImageStatus): List<ImageListItem> = items.value.filter { it.status == status }
